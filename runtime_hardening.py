"""Runtime hardening for legacy bot-vnext.

Applies performance, queue, cancellation and media-cleanup fixes without
requiring a rewrite of the existing downloader/handler pipeline.
"""
...
async def _strip_media_metadata(path):
    ...
    fd, tmp_name = tempfile.mkstemp(
        prefix=f".meta_clean_{src.stem}_",
        suffix=src.suffix,
        dir=str(src.parent),
    )
    os.close(fd)
    ...
    try:
        ...
        if proc.returncode != 0 or not tmp.exists() or tmp.stat().st_size < 1000:
            raise RuntimeError(
                ...
                if stderr else "metadata cleanup failed"
            )
        os.replace(tmp, src)
    except Exception:
        try:
...
def _patch_cancel_handler():
    ...
    original = getattr(handlers, "cancel_task_db")
    ...
    def cancel_task_and_runtime(task_id):
        ...
        if ok:
            try:
                handlers.CANCELLED_TASKS.add(str(task_id))
            except Exception:
                pass
            try:
                queue_worker.cancel_running_task(task_id)
            except Exception:
                pass
            try:
                handlers.cancel_upload_task(task_id)
            except Exception:
                pass
        return ok
