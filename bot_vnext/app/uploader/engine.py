...
class UploadEngine:
        ...
        *,
        ...
        if original_size > MAX_UPLOAD_BYTES:
            ...
            if normalized not in {"video", "document", "file", "doc"}:
                raise UploadError(
                    f"File is larger than 2000 MiB and automatic splitting is only supported for video/document uploads: {path}"
                )
            ...
            try:
                ...
                for index, part in enumerate(parts, 1):
                    ...
                    if part_size > MAX_UPLOAD_BYTES:
                        raise UploadError(
                            f"Generated part is still larger than 2000 MiB ({part_size / 1024 / 1024:.1f} MiB): {part}"
                        )

                    async def part_progress(percent, current, _total, speed, eta, *, base=completed_bytes, psize=part_size):
                        if progress:
                            overall_current = base + min(current, psize)
                            overall_percent = (overall_current * 100.0 / total_bytes) if total_bytes else percent
                            result = progress(overall_percent, overall_current, total_bytes, speed, eta)
                            if asyncio.iscoroutine(result):
                                await result

                    uploaded = await self._upload_single(
                        task,
                        part,
                        chat_id=chat_id,
                ...
                if progress:
                    result = progress(100.0, total_bytes, total_bytes, 0.0, 0)
                    if asyncio.iscoroutine(result):
                        await result
                return uploaded
            finally:
    ...
    async def _split_large_video(self, task: TaskContext, source: Path, parts_dir: Path) -> list[Path]:
        ...
        logger.info(
            "splitting large upload file=%s size=%.1fMiB duration=%.1fs target_segment=%.1fs",
            source,
            size / 1024 / 1024,
            duration,
            target_seconds,
        ...
        command = [
            ...
            "-c", "copy",
            "-f", "segment",
            "-segment_time", f"{target_seconds:.3f}",
            "-reset_timestamps", "1",
            "-movflags", "+faststart",
            output_pattern,
        ]
        ...
        if oversized:
            ...
            self._cleanup_parts(parts_dir, parts)
            raise UploadError(
                f"FFmpeg split produced an oversized part ({biggest:.1f} MiB). Source bitrate/keyframes prevent safe automatic splitting."
            )
        return parts
