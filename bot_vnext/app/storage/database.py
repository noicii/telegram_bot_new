...
class Database:
    ...
    def _init_sync(self):
        ...
        try:
            ...
            CREATE TABLE IF NOT EXISTS tasks (
                ...
                status TEXT NOT NULL DEFAULT 'queued', url TEXT, file_path TEXT,
                title TEXT, caption TEXT, thumbnail TEXT, chat_id INTEGER,
                message_id INTEGER, preset TEXT, mode TEXT, source TEXT,
                provider TEXT, resolution TEXT, batch_id TEXT, batch_total INTEGER DEFAULT 0,
                priority INTEGER DEFAULT 0, retry_count INTEGER DEFAULT 0,
                max_retries INTEGER DEFAULT 3, progress REAL DEFAULT 0,
                speed REAL DEFAULT 0, eta INTEGER DEFAULT 0, error TEXT,
                metadata TEXT, created_at REAL NOT NULL, started_at REAL,
    ...
    def _create_task_sync(self, task_id, task_type, data):
        ...
        try:
            ...
            """, (
                task_id, task_type, data.get("url"), data.get("file_path"), data.get("title"),
                data.get("caption"), data.get("thumbnail"), data.get("chat_id"),
                data.get("message_id"), data.get("preset"), data.get("mode"), data.get("source"),
                data.get("provider"), data.get("resolution"), data.get("batch_id"),
                data.get("batch_total", 0), data.get("priority", 0), 0,
                data.get("max_retries", 3), metadata, now, now
            ))
            conn.commit()
        finally:
            conn.close()
    ...
    def _get_status_sync(self, statuses):
        ...
        conn = self._connect()
        try:
            rows = conn.execute(f"SELECT * FROM tasks WHERE status IN ({marks}) ORDER BY priority DESC, created_at ASC", statuses).fetchall()
            return [self._row(r) for r in rows]
        finally:
            conn.close()

    async def get_tasks(self, statuses: tuple[str, ...] | None = None, limit: int = 25):
        return await asyncio.to_thread(self._get_tasks_sync, statuses, limit)

    def _get_tasks_sync(self, statuses, limit):
        conn = self._connect()
        try:
            if statuses:
                marks = ",".join("?" for _ in statuses)
                rows = conn.execute(f"SELECT * FROM tasks WHERE status IN ({marks}) ORDER BY updated_at DESC LIMIT ?", (*statuses, int(limit))).fetchall()
            else:
    ...
    async def clear_finished(self) -> int:
        return await asyncio.to_thread(self._clear_finished_sync)

    def _clear_finished_sync(self):
        conn = self._connect()
        try:
            cur = conn.execute("DELETE FROM tasks WHERE status IN ('completed', 'failed', 'cancelled')")
            conn.commit()
            return cur.rowcount
        finally:
            conn.close()
    ...
    def _counts_sync(self):
        conn = self._connect()
        try:
            rows = conn.execute("SELECT task_type, status, COUNT(*) AS count FROM tasks GROUP BY task_type, status").fetchall()
            result = {}
            for row in rows:
