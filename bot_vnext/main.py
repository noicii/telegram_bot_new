...
from app.downloader.method_store import (
    get_method, set_method, get_default_method, set_default_method,
    next_method, method_label, METHODS,
)
...
class V2Bot:
    def __init__(self, client: Client):
        ...
        self.dashboard_locks: dict[int, asyncio.Lock] = {}
        self.dashboard_last_edit: dict[int, float] = {}

    def _dashboard_lock(self, chat_id: int) -> asyncio.Lock:
        return self.dashboard_locks.setdefault(chat_id, asyncio.Lock())
    ...
    async def status_cmd(self, client, message):
        if owner_only(message):
            await self.show_dashboard(message.chat.id, source_message=message)

    async def show_dashboard(self, chat_id: int, source_message=None):
        if not self.pipeline:
            if source_message:
                await source_message.reply_text("❌ V2 pipeline is not running.")
            return
        message_id = self.dashboard_messages.get(chat_id)
        if message_id:
            ok = await self._render_live_dashboard(chat_id, message_id, force=True)
            if ok:
                return
            self.dashboard_messages.pop(chat_id, None)
        if source_message:
            try:
                msg = await source_message.reply_text("📊 **LIVE DOWNLOAD / UPLOAD**\n\n⏳ Loading live status…")
            except Exception:
                return
        else:
            try:
                msg = await self.client.send_message(chat_id, "📊 **LIVE DOWNLOAD / UPLOAD**\n\n⏳ Loading live status…")
            except Exception:
                return
        self.dashboard_messages[chat_id] = msg.id
        await self._render_live_dashboard(chat_id, msg.id, force=True)
    ...
    async def send_queue(self, message):
        ...
        if not active:
            await message.reply_text(
                ...
                reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("📊 Status", callback_data="v2:status")]])
            )
            return
        lines = ["📋 **V2 QUEUE**", ""]
        buttons = []
        ...
        buttons.append([InlineKeyboardButton("📊 Live Status", callback_data="v2:status"), InlineKeyboardButton("🔄 Refresh", callback_data="v2:queue")])
    ...
    async def retry_cmd(self, client, message):
        ...
        if len(parts) != 2:
            await message.reply_text("Usage: `/retry TASK_ID`\n\nDownloads retry with the current `/method` selection and get a fresh 3-attempt limit.")
            return
        if not self.pipeline:
            await message.reply_text("❌ V2 pipeline is offline.")
            return
    ...
    async def settings_cmd(self, client, message):
        if owner_only(message):
            await self.send_settings(message)
    ...
    async def send_settings(self, message):
        ...
        await message.reply_text(
            ...
            reply_markup=InlineKeyboardMarkup([
                ...
                [InlineKeyboardButton("📊 Live Status", callback_data="v2:status"), InlineKeyboardButton("📋 Queue", callback_data="v2:queue")],
    ...
    async def health_cmd(self, client, message):
        if owner_only(message):
            await self.send_health(message)

    async def send_health(self, message):
        checks = [
            ("Pipeline", bool(self.pipeline and self.pipeline._started)),
            ("SQLite DB", Path(ROOT / "bot_vnext.db").exists()),
    ...
    async def callback(self, client, query):
        ...
        if data == "v2:status":
            await query.answer()
            self.dashboard_messages[query.message.chat.id] = query.message.id
            await self._render_live_dashboard(query.message.chat.id, query.message.id, force=True)
            return
        if data == "v2:queue":
            await query.answer()
        ...
        if data in {"v2:health", "v2:healthmenu"}:
            ...
            return
        if data == "v2:methodmenu" and not self.sessions.get(query.from_user.id):
            await query.answer()
        ...
        if data.startswith("v2:cancel:"):
            ...
            if ok:
                await self._render_live_dashboard(query.message.chat.id, self.dashboard_messages.get(query.message.chat.id), force=True)
            else:
                await self.send_queue(query.message)
            return
        ...
        if data.startswith("v2:t:"):
            ...
            session["selected"].discard(index) if index in session["selected"] else session["selected"].add(index)
            await query.answer()
            await self.render_selection(query.message, session)
            return
        if data == "v2:all":
    ...
    async def enqueue_selected(self, message, session, selected):
        ...
        dashboard = await message.edit_text(
        ...
        for index in selected:
            ...
            metadata = {
                ...
                "headers": {"Referer": item.get("source_url")} if item.get("source_url") else {},
            }
            metadata = {k: v for k, v in metadata.items() if v}
            payload = {
                "task_type": "download", "url": item["url"], "filename": filename,
                "chat_id": message.chat.id, "caption": item.get("title") or session["series"],
                "thumbnail": str(THUMB_PATH) if THUMB_PATH.is_file() else None,
                "mode": "video", "title": item.get("title"), "provider": item.get("source"),
                "resolution": item.get("resolution"), "metadata": metadata,
            }
            try:
                await self.pipeline.submit(task_id, payload)
            except Exception:
                logger.exception("queue submit failed for %s", task_id)
        self.sessions.pop(message.from_user.id, None)
        await self._render_live_dashboard(message.chat.id, dashboard.id, force=True)

    async def _render_live_dashboard(self, chat_id: int, message_id: int | None = None, force: bool = False) -> bool:
        if not self.pipeline or not message_id:
            return False
        lock = self._dashboard_lock(chat_id)
        async with lock:
            now = asyncio.get_running_loop().time()
            if not force and now - self.dashboard_last_edit.get(chat_id, 0.0) < 1.2:
                return True
            ...
            download_rows = [r for r in rows if r.get("task_type") == "download"]
            upload_rows = [r for r in rows if r.get("task_type") == "upload"]

            if download_rows:
                lines.append("📥 **DOWNLOADING**")
                for index, row in enumerate(download_rows[:8], 1):
                    task_id = str(row.get("id"))
                    live = self.progress_cache.get(task_id, {})
                    pct = float(row.get("progress") or live.get("percent") or 0)
                    current = live.get("current", 0)
                    total = live.get("total", 0)
                    speed = live.get("speed", row.get("speed", 0))
                    eta = live.get("eta", row.get("eta", 0))
                    details = live.get("details") or {}
                    title = str(row.get("title") or row.get("url") or task_id).strip()[:48]
                    resolution = str(row.get("resolution") or details.get("resolution") or "").strip()
                    title_line = f"{title} • {resolution}" if resolution else title
                    lines += [
                        f"{index}️⃣ **{title_line}**",
                        f"{_bar(pct)} **{pct:.0f}%**",
                        f"📦 {_fmt_bytes(current)} / {_fmt_bytes(total)} • ⚡ {_fmt_speed(speed)} • ETA {_fmt_eta(eta)}",
                    ]
                    if details.get("hls_total"):
                        retries = int(details.get("hls_retries") or 0)
                        retry_text = f" • 🔁 {retries} retries" if retries else ""
                        lines.append(f"🧩 HLS: {int(details.get('hls_completed') or 0)} / {int(details.get('hls_total') or 0)} segments{retry_text}")
                    lines.append("")
            else:
                lines.append("📥 **DOWNLOADING**\nNo active downloads.\n")
            ...
            if upload_rows:
                ...
                for index, row in enumerate(upload_rows[:8], 1):
                    task_id = str(row.get("id"))
                    live = self.progress_cache.get(task_id, {})
                    pct = float(row.get("progress") or live.get("percent") or 0)
                    current = live.get("current", 0)
                    ...
                    if int(details.get("parts") or 1) > 1:
                        part = int(details.get("part") or 1)
                        parts = int(details.get("parts") or 1)
                        lines.append(f"📦 Part {part}/{parts}")
                    lines.append("")
            else:
                lines.append("📤 **UPLOADING**\nNo active uploads.\n")

            total_current = sum(float(self.progress_cache.get(str(r.get("id")), {}).get("current") or 0) for r in rows)
            total_size = sum(float(self.progress_cache.get(str(r.get("id")), {}).get("total") or 0) for r in rows)
            if total_size:
                overall = total_current * 100 / total_size
                lines += [f"📊 **Overall active:** {_fmt_bytes(total_current)} / {_fmt_bytes(total_size)} • **{overall:.0f}%**"]

            disk = shutil.disk_usage(DOWNLOAD_DIR)
            used = disk.total - disk.free
            pct_disk = used * 100 / disk.total if disk.total else 0
            icon = "🔴" if pct_disk >= 90 else "🟠" if pct_disk >= 80 else "🟢"
            lines.append(f"{icon} **Disk:** {used / (1024 ** 3):.1f}/{disk.total / (1024 ** 3):.1f} GB used • {disk.free / (1024 ** 3):.1f} GB free ({pct_disk:.0f}%)")
            ...
            markup = InlineKeyboardMarkup([
                [InlineKeyboardButton("🔄 Refresh", callback_data="v2:status"), InlineKeyboardButton("📋 Queue", callback_data="v2:queue")],
            ])
            try:
                ...
                self.dashboard_last_edit[chat_id] = now
                return True
            except Exception as exc:
                logger.debug("dashboard edit failed chat=%s message=%s: %s", chat_id, message_id, exc)
                return False
    ...
    async def on_progress(self, kind, task_id, percent=None, current=None, total=None, speed=None, eta=None, details=None):
        ...
        self.progress_cache[task_id] = {
            "kind": kind, "percent": percent, "current": current or 0, "total": total or 0,
            "speed": speed or 0, "eta": eta or 0, "details": details or {},
        }
        try:
            row = await self.pipeline.db.get_task(task_id) if self.pipeline else None
            if row and row.get("chat_id"):
                message_id = self.dashboard_messages.get(int(row["chat_id"]))
                if message_id:
                    await self._render_live_dashboard(int(row["chat_id"]), message_id)
        except Exception:
    ...
    async def on_complete(self, task_id):
        ...
        try:
            ...
            if row and row.get("chat_id"):
                await self._render_live_dashboard(int(row["chat_id"]), self.dashboard_messages.get(int(row["chat_id"])), force=True)
        except Exception:
            pass
